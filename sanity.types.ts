...
// Query: *[_type == 'startup' && defined(slug.current) && !defined($search) || title match $search || category match $search || author->name match $search] | order(_createdAt desc) {  _id,   title,  author -> {    _id, name, bio, email, username, image  },   slug,   _createdAt,   views,   description,   image,   category}
...
// Variable: STARTUP_VIEWS_BY_ID
// Query: *[_type == "startup" && _id == $id][0] {      _id, views    }
export type STARTUP_VIEWS_BY_IDResult = {
  _id: string;
  views: number | null;
} | null;
...
// Query: *[_type == "startup" && author._ref == $id] | order(_createdAt desc) {    _id,     title,    slug,     _createdAt,     author -> {      _id, name, bio, email, username, image    },      views,     description,     category,    image,   }
